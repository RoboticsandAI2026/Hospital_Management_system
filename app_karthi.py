# === app.py - Streamlit Web Interface for Anomaly Detection and Forecasting ===
import os
...
def run():
    ...
    uploaded_file = st.sidebar.file_uploader("Upload patient vitals CSV file", type=["csv"])

    # Demo data option
    use_demo_data = st.sidebar.checkbox("Use demo data", value=True if not uploaded_file else False)
    ...
    if analysis_type == "Anomaly Detection":
        threshold_factor = st.sidebar.slider("Anomaly Threshold Factor", min_value=1.0, max_value=5.0, value=2.0, step=0.1,
                                           help="Higher values will detect fewer but more significant anomalies")
        ...
        model_path = st.sidebar.text_input("Forecast Model Path", 
                                          value="best_model.h5",
                                          help="Path to the trained forecasting model")
